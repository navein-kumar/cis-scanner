<#
.SYNOPSIS
    CIS Benchmark Scanner v2.5.1 (Silent Mode)
...
$LogFile = Join-Path $env:TEMP "cis_scanner_debug.log"
"[$(Get-Date)] CIS Scanner Started" | Out-File $LogFile -Append
...
"[$(Get-Date)] Script Dir: $ScriptDir" | Out-File $LogFile -Append
...
if (!(Test-Path $PolicyFolder)) {
    "[$(Get-Date)] ERROR: Policy folder not found: $PolicyFolder" | Out-File $LogFile -Append
    throw "Policy folder not found: $PolicyFolder"
}
"[$(Get-Date)] Policy folder OK" | Out-File $LogFile -Append
...
"[$(Get-Date)] Detected OS: $DetectedOS" | Out-File $LogFile -Append
...
if (!(Test-Path $LocalPolicy)) {
    "[$(Get-Date)] ERROR: Policy file not found: $LocalPolicy" | Out-File $LogFile -Append
    throw "Policy file not found: $LocalPolicy"
}
"[$(Get-Date)] Policy file OK: $PolicyFileName" | Out-File $LogFile -Append
...
# ============== RUN SCAN ==============
"[$(Get-Date)] Parsing policy..." | Out-File $LogFile -Append
$Checks = Parse-WazuhPolicy -FilePath $LocalPolicy
"[$(Get-Date)] Parsed $($Checks.Count) checks" | Out-File $LogFile -Append

if ($Checks.Count -eq 0) {
    throw "No checks parsed from policy file!"
}

"[$(Get-Date)] Running scan..." | Out-File $LogFile -Append
...
foreach ($check in $Checks) {
    ...
    $Results += [PSCustomObject]@{
        ...
        Condition = $check.condition; Rules = $check.rules; RuleResults = $eval.RuleResults
        Description = $meta.description; Rationale = $meta.rationale; Remediation = $meta.remediation; Compliance = ($meta.compliance -join "; ")
    }
}

"[$(Get-Date)] Scan complete: $PassCount Pass, $FailCount Fail" | Out-File $LogFile -Append
...
.stat-lbl{color:#666;font-size:12px;margin-top:5px;text-transform:uppercase}
...
<table id="results"><thead><tr><th style="width:80px">CIS ID</th><th>Title</th><th style="width:80px">Status</th><th style="width:70px">Details</th></tr></thead><tbody>
$htmlRows
</tbody></table>
...
"[$(Get-Date)] Reports saved" | Out-File $LogFile -Append
"[$(Get-Date)] HTML: $HTMLReport" | Out-File $LogFile -Append
"[$(Get-Date)] CSV: $CSVReport" | Out-File $LogFile -Append

} catch {
    # Log error
    "[$(Get-Date)] ERROR: $($_.Exception.Message)" | Out-File $LogFile -Append
    "[$(Get-Date)] Line: $($_.InvocationInfo.ScriptLineNumber)" | Out-File $LogFile -Append
    
    # Create error file
    $ErrorFile = Join-Path $ScriptDir "CIS_ERROR.txt"
    @"
==================== CIS SCANNER ERROR ====================
Time: $(Get-Date)
